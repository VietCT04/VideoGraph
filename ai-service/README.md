...
The current `pipeline/metadata.py` and `pipeline/segmentation.py` modules provide a
dependency-free implementation of issue #3. Real media probing and scene detection can
be supplied behind the same boundaries later; fixture metadata and timestamp inputs
make the chunking behavior deterministic in local tests.
